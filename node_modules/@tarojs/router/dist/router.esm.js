import { eventCenter, Current, stringify, createPageConfig, CurrentReconciler, requestAnimationFrame } from '@tarojs/runtime';
...
function createRouter(app, config, framework) {
    ...
    for (let i = 0; i < config.routes.length; i++) {
        ...
        const path = addLeadingSlash$1(route.path);
        routes.push({
    ...
    const render = async (location, action) => {
        ...
        const pageConfig = config.routes.find(r => {
            const path = addLeadingSlash$1(r.path);
        ...
        if (shouldLoad) {
            const el = (_b = element.default) !== null && _b !== void 0 ? _b : element;
            const config = Object.assign({}, pageConfig);
            delete config['path'];
            delete config['load'];
            const page = createPageConfig(enablePullDownRefresh ? (_c = CurrentReconciler.createPullDownComponent) === null || _c === void 0 ? void 0 : _c.call(CurrentReconciler, el, location.pathname, framework) : el, location.pathname + stringify(qs()), {}, config);
            loadPage(page, pageConfig);
        }
    };
    if (history.location.pathname === '/') {
        history.replace(routes[0].path + history.location.search);
    }
